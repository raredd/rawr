...
`click_shape`   | add shapes interactively
`click_text`    | add text interactively
...
`search_df`   | search data frames
`search_hist` | search history
...
```{r}
search_df("Posch", df, make, var = 1)  # inexact matching
```
...
`dlt_table`        | dlt table  
`local_coxph_test` | local coxph p-value
`match_ctc`        | convert ctc toxicity codes to descriptions
`moods_test`       | mood's median test
`power_cv`         | coefficient of variation power calculation
...
power_cv(n = NULL, f = 1.25, cv = 0.2, sig.level = 0.05, power = 0.8, 
         distribution = "normal")
```
